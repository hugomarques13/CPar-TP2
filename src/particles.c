...
void spec_advance( t_species* spec, t_emf* emf, t_current* current )
{
    ...
    MPI_Scatterv(
        (rank == 0) ? spec->part : NULL,
        counts, displs, MPI_PART,
        local_part, local_np, MPI_PART,
        0, MPI_COMM_WORLD
    );
    
    // ============= 5. ALOCAR BUFFER DE CORRENTE LOCAL =============
    // Criar estrutura current local para dep_current_zamb
    t_current local_current;
    int current_size = 1 + nx0 + 2;  // gc[0]=1, gc[1]=2
    local_current.J_buf = (float3*)calloc(current_size, sizeof(float3));
    local_current.J = local_current.J_buf + 1;
    local_current.nx = nx0;
    local_current.gc[0] = 1;
    local_current.gc[1] = 2;
    ...
    for (int i = 0; i < local_np; i++) {
        ...
        // interpolate fields
        interpolate_fld(emf_E_part, emf_B_part, p, &Ep, &Bp);
        ...
        // Função ltrim (supondo existente) - se não, use:
        di = (x1 >= 1.0f) - (x1 < 0.0f);  // Equivalente a ltrim
        x1 -= di;
        ...
        // Corrigido: passa &local_current (ponteiro para t_current)
        dep_current_zamb(p->ix, di, p->x, dx, qnx, qvy, qvz, &local_current);
    ...
    // Todos os ranks alocam buffer para corrente global
    float3 *global_J_buf = NULL;
    if (rank == 0) {
        global_J_buf = current->J_buf;
    } else {
        global_J_buf = (float3*)malloc(current_size * sizeof(float3));
    }
    
    // Reduzir corrente (soma de todos os buffers locais)
    MPI_Reduce(local_current.J_buf, global_J_buf, current_size * 3, 
    ...
    // Broadcast número de partículas atualizado
    MPI_Bcast(&spec->np, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&spec->iter, 1, MPI_INT, 0, MPI_COMM_WORLD);
    
    // Broadcast partículas (todos precisam de cópia atualizada)
    if (rank == 0) {
        // Root já tem dados em spec->part
    } else {
        // Workers realocam se necessário
        if (spec->np > spec->buffer_size) {
            spec->part = (t_part*)realloc(spec->part, spec->np * sizeof(t_part));
            spec->buffer_size = spec->np;
        }
    }
