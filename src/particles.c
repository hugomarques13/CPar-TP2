...
void spec_advance( t_species* spec, t_emf* emf, t_current* current )
{
    ...
    // ============= 1. BROADCAST DE PARÂMETROS =============
    // Root coleta valores, workers recebem
    ...
    // Cálculo balanceado de partículas por rank
    int base = spec_np / size;
    int rem = spec_np % size;
    int local_np = base + (rank < rem ? 1 : 0);
    ...
    if (rank == 0) {
        ...
        // Workers só precisam saber seu próprio tamanho
        counts = (int*)malloc(sizeof(int));  // Não usado, mas evita segfault
        displs = (int*)malloc(sizeof(int));
    }
    
    // ============= 3. BROADCAST DE CAMPOS =============
    // Workers precisam alocar buffers para campos
    int emf_size = nx0 + 1;  // +1 para interpolação?
    float3 *emf_E_part = (float3*)malloc(emf_size * sizeof(float3));
    float3 *emf_B_part = (float3*)malloc(emf_size * sizeof(float3));
    
    if (rank == 0) {
        // Root já tem os dados em emf->E_part, emf->B_part
        memcpy(emf_E_part, emf->E_part, emf_size * sizeof(float3));
        memcpy(emf_B_part, emf->B_part, emf_size * sizeof(float3));
    }
    
    // Broadcast dos campos (agora buffers alocados em todos os ranks)
    MPI_Bcast(emf_E_part, emf_size * sizeof(float3), MPI_BYTE, 0, MPI_COMM_WORLD);
    MPI_Bcast(emf_B_part, emf_size * sizeof(float3), MPI_BYTE, 0, MPI_COMM_WORLD);
    
    // ============= 4. SCATTER DE PARTÍCULAS =============
    // Alocar buffer local para partículas
    t_part *local_part = (t_part*)malloc(local_np * sizeof(t_part));
    
    // Criar tipo MPI para t_part (melhor que MPI_BYTE)
    MPI_Datatype MPI_PART;
    MPI_Type_contiguous(sizeof(t_part), MPI_BYTE, &MPI_PART);
    MPI_Type_commit(&MPI_PART);
    
    // Scatter balanceado
    MPI_Scatterv(
        (rank == 0) ? spec->part : NULL,  // Buffer fonte (apenas root)
        counts,      // Número por rank
        displs,      // Deslocamentos
        MPI_PART,    // Tipo de dados
        local_part,  // Buffer destino
        local_np,    // Quantas receber
        MPI_PART,    // Tipo de dados
        0,           // Root
        MPI_COMM_WORLD
    );
    
    // ============= 5. ALOCAR BUFFER DE CORRENTE LOCAL =============
    int current_size = 1 + nx0 + 2;  // gc[0]=1, gc[1]=2
    float3 *local_J_buf = (float3*)calloc(current_size, sizeof(float3));  // calloc zera
    float3 *local_J = local_J_buf + 1;  // Offset como em current->J
    
    // ============= 6. PROCESSAMENTO LOCAL =============
    double energy = 0.0;
    
    for (int i = 0; i < local_np; i++) {
        // ... código do loop Boris (igual ao seu) ...
        // MAS usando variáveis locais:
        // tem, dt_dx, qnx, spec_q, nx0
        // emf_E_part, emf_B_part
        // local_part[i]
        // local_J
    ...
    // Reduzir corrente: todos contribuem, root recebe soma
    // Primeiro garantir que current->J existe em todos os ranks
    float3 *global_J_buf = NULL;
    if (rank == 0) {
        global_J_buf = current->J_buf;  // Root usa buffer existente
    } else {
        global_J_buf = (float3*)malloc(current_size * sizeof(float3));
    }
    
    MPI_Reduce(local_J_buf, global_J_buf, current_size * 3, MPI_FLOAT, 
               MPI_SUM, 0, MPI_COMM_WORLD);
    
    // Gather das partículas atualizadas
    MPI_Gatherv(
        local_part,    // Dados locais
        local_np,      // Quantas enviar
        MPI_PART,      // Tipo
        (rank == 0) ? spec->part : NULL,  // Destino (apenas root)
        counts,        // Mesmos counts
        displs,        // Mesmos displs
        MPI_PART,      // Tipo
        0,             // Root
        MPI_COMM_WORLD
    );
    
    // ============= 8. PÓS-PROCESSAMENTO APENAS NO ROOT =============
    if (rank == 0) {
        // Atualizar energia
        spec->energy = spec->q * spec->m_q * total_energy * spec->dx;
        spec->iter += 1;
        
        // Copiar corrente reduzida para estrutura atual
        memcpy(current->J_buf, global_J_buf, current_size * sizeof(float3));
        
        // Tratamento de condições de contorno (seu código existente)
    ...
    // ============= 9. BROADCAST DO ESTADO ATUALIZADO =============
    // Para sincronizar todos os ranks para próxima iteração
    ...
    if (rank != 0 && global_J_buf) free(global_J_buf);
